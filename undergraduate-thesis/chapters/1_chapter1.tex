...
离散数学，尤其是图论，为理解和建模这种结构演化提供了坚实的理论基础。其中，图编辑距离（Graph Edit Distance, GED）作为一种经典的图相似性度量，能够将两个图之间的差异转化为一系列可解释的原子操作（如顶点/边的插入、删除、替换），并量化其最小转换代价 \cite{blumenthal2020exact}。GED不仅度量了变化的幅度，其蕴含的编辑路径更揭示了图结构演化的具体模式。这为我们提供了一种强有力的数学工具：将图持续学习中“记忆与适应”的平衡问题，转化为对图结构演化轨迹的量化分析与针对性干预。
...
一个图 $G$ 可以定义为一个有序二元组 $G=(V,E)$，其中 $V$ 是一个有限的、非空的集合，称为顶点集，$E$ 是顶点集 $V$ 中无序对的集合
...
当我们需要判断两个图在结构上是否“完全相同”时，就需要用到离散数学中图同构的概念。同构是图论中最深刻的等价关系，也是许多图算法（如图匹配、图数据库查询）的核心。作为图同构检测的离散数学基石，Weisfeiler-Lehman 图核定义了衡量图相似性的经典准则 \cite{shervashidze2011weisfeiler}，这一理论也常被用于分析图神经网络的表达能力上界 \cite{xu2019powerful}。

两个图 $G=(V,E)$ 和 $H=(U,F)$ 是同构的，即 $G\cong H$，当且仅当存在一个双射函数 $\varphi:V \rightarrow U$，使得
对 $V$ 中任意两个顶点 $u,v$，
\begin{align}
[u,v]\in E \iff [\varphi(u),\varphi(v)]\in F.
\end{align}
这样的函数 $\varphi$ 称为从 $G$ 到 $H$ 的一个同构映射。对于有向图和标记图，则条件要分别加强到有向边相同和标签相同。

同构关系表明，两个图虽然在表面表示上可能不同（顶点编号、空间布局），但其连接拓扑结构完全一致。

\subsection{子图同构与图匹配问题}

一个更普遍且在实践中至关重要的难题是子图同构问题，它询问一个图是否包含另一个图的同构副本。在处理大规模图的子图匹配问题时，典型的工业级标准算法包括 VF2 算法 \cite{cordella2004sub}。
...
在动态图序列的分析中，我们不仅需要识别结构的定性与定量变化，更需要一种能够精确量化这种变化、并能揭示变化细节的数学工具。图编辑距离（Graph Edit Distance, GED）正是这样一种工具，它将图之间的差异转化为一系列可解释的原子操作，并通过最小化操作总代价来定义距离。Bunke 从离散数学角度证明了 GED 与最大公共子图之间的等价性，为该度量提供了坚实的理论支撑 \cite{bunke1997relation}。
...
图编辑距离的精确计算是 \textsc{NP}-hard 问题 \cite{blumenthal2020exact}，且对于一般的图，其判定问题（给定两个图和阈值 $K$，判断 $\mathrm{GED}$ 是否小于等于 $K$）是 \textsc{NP}-完全的。
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{基于搜索的精确算法：} 例如 A$^*$ 算法，它利用启发式函数剪枝搜索空间。近年来，Mata* 通过结合可学习的节点匹配与 A$^*$ 算法，提升了近似计算的效率 \cite{liu2023mata}。

  \item \textbf{整数线性规划（ILP）模型：} 将 $\mathrm{GED}$ 计算建模为一个整数线性规划问题 \cite{blumenthal2020exact}，利用优化求解器（如 Gurobi、CPLEX）求解。

  \item \textbf{近似与启发式算法：} 这是处理大规模图的主要途径。
        \begin{itemize}
          \item \textbf{二分图匹配法：} 将顶点映射问题转化为二分图最小权匹配问题 \cite{riesen2009approximate}，通过忽略部分边结构获得多项式时间的近似解。
          \item \textbf{图神经网络方法：} 利用可微近似研究 \cite{wallnig2024differentiable} 或针对编辑距离的节点匹配网络 \cite{moscatelli2024graph, li2019graph}，将离散的匹配问题转化为可微的深度学习问题。
        \end{itemize}
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 基于结构的回放增强：在回放一个子图记忆项时，并非孤立地输入该子图，而是将其“植入”
        到当前快照 $G_t$ 的一个相似上下文中，以模拟该核心结构在新环境下的作用，增强知识的可迁移性。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 计算拓扑加权的 Fisher 信息矩阵：  
        在标准的 EWC 中，参数 $\theta_i$ 的重要性正比于 Fisher 信息矩阵对角线元素 $F_i$。在图卷积网络的背景下，参数的稳定性对泛化性能至关重要 \cite{verma2019stability}。我们引入拓扑权重进行修正：
        \begin{itemize}[leftmargin=*,nosep]
          ...
          \item 然后，计算拓扑加权 Fisher 信息：
                ...
                \]
                这里 $\alpha(v)$ 是顶点活跃度。关键洞察：对稳定顶点（低 $\alpha(v)$）预测正确的参数更为重要，因为这些顶点表征了网络的静态核心。
        \end{itemize}
...
\chapter{算法实现与复杂性分析}
鉴于GED精确计算的NP-hard特性，我们首先讨论高效的近似计算优化方法，随后描述整体算法流程，并分析其时间与空间复杂度。

\section{近似计算优化}
精确计算GED对于大规模图是不可行的。因此，我们采用并改进一种经典的近似算法——基于二分图匹配的GED计算（Bipartite Graph Matching for GED, BGM-GED）\cite{riesen2009approximate}，其核心思想是将复杂的图匹配问题分解为顶点匹配与局部结构评估相结合的过程。
...
\item 求解最小代价分配：\\
使用匈牙利算法（Hungarian Algorithm）或更快的 Jonker-Volgenant 算法求解上述代价矩阵的最小代价分配，得到最优的顶点映射 $\Phi$ \cite{riesen2009approximate}。
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \begin{itemize}
    ...
    \item 对于每条边 $e_2=(u_j,u_{j'})\in E_2$，如果其端点不是由 $\Phi$ 从 $G_1$ 映射而来，则需在 $G_1$ 中插入此边，代价为 $C_{ie}$。
  \end{itemize}
...
\begin{itemize}[leftmargin=*,nosep]
  \item \textbf{热启动映射：} 使用上一轮计算得到的映射作为初始解，通过局部调整来求解 $\Phi_{t-1,t}$。
  \item \textbf{局部搜索：} 当变化很小时，可以限制搜索空间，仅考虑对可能发生变化的顶点进行重新匹配。
\end{itemize}

\subsubsection*{复杂性分析（单次 BGM-GED）}
\begin{itemize}[leftmargin=*,nosep]
  \item \textbf{时间复杂性：} 步骤 1 构造代价矩阵需要 $O(|V_1|\cdot|V_2|\cdot d)$ 时间。步骤 2 匈牙利算法的时间复杂度为 $O\!\bigl(\max(|V_1|,|V_2|)^3\bigr)$ \cite{riesen2009approximate}。步骤 3 需要 $O(|E_1|+|E_2|)$ 时间。因此，总时间复杂度为 $O(n^3+m)$，其中 $n=\max(|V_1|,|V_2|)$, $m=\max(|E_1|,|E_2|)$。
  \item \textbf{空间复杂性：} 主要存储代价矩阵，需要 $O(|V_1|\cdot|V_2|)$ 空间。
\end{itemize}

\chapter{总结与展望}
\section{结论}
本文深入探讨了离散数学，特别是图编辑距离，在图持续学习这一前沿领域中的应用。面对动态图序列中灾难性遗忘的核心挑战，我们提出一个基于 GED 的分层动态建模框架，并通过设计 GGR 回放策略与 TA-EWC 正则化策略，实现了对图结构变化的精细化度量与适应。本文工作表明，将离散数学的严谨理论与机器学习的前沿需求相结合，能够催生具有更强可解释性和性能潜力的新方法。

\section{未来工作}
尽管本研究提出了一个系统的框架，但仍有诸多方向值得在未来进一步探索：

\begin{enumerate}[label=\arabic*.]
  \item \textbf{GED 计算的进一步加速：} 探索基于图神经网络的端到端预测或层次化匹配。
  \item \textbf{更复杂的动态场景建模：} 研究连续时间动态图中的“瞬时编辑距离”。
  \item \textbf{几何度量工具的应用：} 利用 Gromov-Wasserstein 学习等最优传输理论定义更具几何性质的图匹配度量 \cite{xu2019gromov}。
  \item \textbf{策略的融合与自动化：} 动态调整回放比例、正则化强度等超参数。
\end{enumerate}
...
\bibitem{blumenthal2020exact} Blumenthal, D. B., \& Gamper, J. (2020). On the exact computation of the graph edit distance. \textit{Pattern Recognition Letters}, 134, 46-57.
\bibitem{wallnig2024differentiable} Wallnig, J., Brun, L., et al. (2024). A Differentiable Approximation of the Graph Edit Distance. \textit{SPR/SSPR}, 1-10.
\bibitem{moscatelli2024graph} Moscatelli, A., et al. (2024). Graph node matching for edit distance. \textit{Pattern Recognition Letters}, 184, 14-20.
\bibitem{liu2023mata} Liu, J., Zhou, M., et al. (2023). Mata*: Combining learnable node matching with a* algorithm for approximate graph edit distance computation. \textit{ACM CIKM}, 1503-1512.
\bibitem{shervashidze2011weisfeiler} Shervashidze, N., et al. (2011). Weisfeiler-lehman graph kernels. \textit{Journal of Machine Learning Research}, 12(9), 2539-2561.
\bibitem{bunke1997relation} Bunke, H. (1997). On the relation between graph edit distance and maximum common subgraph. \textit{Pattern Recognition Letters}, 18(8), 689-694.
\bibitem{riesen2009approximate} Riesen, K., \& Bunke, H. (2009). Approximate graph edit distance computation by means of bipartite graph matching. \textit{Image and Vision Computing}, 27(7), 950-959.
\bibitem{cordella2004sub} Cordella, L. P., et al. (2004). A (sub) graph isomorphism algorithm for matching large graphs. \textit{IEEE Transactions on PAMI}, 26(10), 1367-1372.
\bibitem{li2019graph} Li, Y., et al. (2019). Graph Matching Networks for Learning the Similarity of Graph Structured Objects. \textit{ICML}, 3835-3845.
\bibitem{verma2019stability} Verma, S., \& Zhang, Z.-L. (2019). Stability and generalization of graph convolutional neural networks. \textit{ACM SIGKDD}, 1539-1548.
\bibitem{xu2019gromov} Xu, H., et al. (2019). Gromov-Wasserstein Learning for Graph Matching and Node Embedding. \textit{ICML}, 6932-6941.
